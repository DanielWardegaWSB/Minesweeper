...
class RevealFields:
    ...
    def _reveal_neighbours(self, x, y):
        if self._is_pos_correct(x, y):
            if (
                self._board_array[x][y].revealed is False
                and self._board_array[x][y].flagged is False
            ):
                if self._board_array[x][y].value in map(str, range(1, 9)):
                    self._reveal_specyfied_field(x, y)

                if (
                    self._board_array[x][y].value
                    == settings.VALUES_OF_BOARD_FIELDS["INITIAL_VALUE_OF_THE_FIELD"]
                ):
                    self._reveal_specyfied_field(x, y)
    ...
    def _reveal_specyfied_field(self, x, y):
        ...
        if self._board_array[x][y].flagged is False:
            ...
            if field_value == settings.VALUES_OF_BOARD_FIELDS["BOMB"]:
                ...
                path_to_icon_image = (
                    self._path_to_icons_with_values
                    + field_value
                    + "."
                    + self._file_format_of_icons_with_values
                )
            else:
                self._number_of_reveal_fields += 1
                path_to_icon_image = settings.IMAGES_WITH_THE_ICONS_OF_FIELDS[
